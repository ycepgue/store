...
var NumberFieldRoot_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ (0, vue.defineComponent)({
	...
	setup(__props, { emit: __emit }) {
		...
		const isDecreaseDisabled = (0, vue.computed)(() => {
			if (require_shared_nullish.isNullish(modelValue.value) || isNaN(modelValue.value)) return false;
			return getNextValue("decrease", modelValue.value) >= modelValue.value;
		});
		const isIncreaseDisabled = (0, vue.computed)(() => {
			if (require_shared_nullish.isNullish(modelValue.value) || isNaN(modelValue.value)) return false;
		...
		function getNextValue(type, from, multiplier = 1) {
			...
			if (stepSnapping.value && !isNaN(stepValue)) {
				const snapped = require_shared_clamp.snapValueToStep(from, min.value, max.value, stepValue);
				if (snapped === from) nextValue = require_NumberField_utils.handleDecimalOperation(operator, from, stepValue * multiplier);
				else {
					const aligned = type === "increase" ? snapped > from ? snapped : require_NumberField_utils.handleDecimalOperation("+", snapped, stepValue) : snapped < from ? snapped : require_NumberField_utils.handleDecimalOperation("-", snapped, stepValue);
					nextValue = multiplier > 1 ? require_NumberField_utils.handleDecimalOperation(operator, aligned, stepValue * (multiplier - 1)) : aligned;
				}
			} else nextValue = require_NumberField_utils.handleDecimalOperation(operator, from, stepValue * multiplier);
